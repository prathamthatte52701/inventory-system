...
module.exports = async function setup(label) {
  ...
  await seedAdmins();
  h.admin = (await h.call('POST', '/auth/login', { email: process.env.ADMIN1_EMAIL, password: process.env.ADMIN1_PASSWORD })).b.token;
  ...
  h.user = (await h.call('POST', '/auth/login', { email: 'bob@test.com', password: 'secret1' })).b.token;
