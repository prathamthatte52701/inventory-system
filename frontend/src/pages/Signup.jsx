...
export default function Signup() {
  ...
  if (done)
    return (
      <div className="mx-auto mt-12 grid w-full max-w-sm gap-4 rounded-xl border border-slate-200 bg-white p-7 shadow-sm">
  ...
  return (
    <form className="mx-auto mt-12 grid w-full max-w-sm gap-4 rounded-xl border border-slate-200 bg-white p-7 shadow-sm" onSubmit={submit} noValidate>
      <div className="grid gap-1">
        ...
        <p className="text-sm text-slate-500">An admin has to approve your account before you can log in.</p>
      </div>
      ...
      <span className="text-center text-sm text-slate-500">Have an account? <Link to="/login" className="font-medium text-primary hover:underline">Log in</Link></span>
    </form>
  );
}
